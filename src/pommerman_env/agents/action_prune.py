...
def move_moving_bombs_to_next_position(prev_obs, obs):
    def is_moving_direction(bomb_pos, direction):
        ...
        if not utility.position_on_board(prev_obs["board"], rev_pos):
            return False
        if (
            prev_obs["bomb_life"][rev_pos] - 1 == obs["bomb_life"][bomb_pos]
            and prev_obs["bomb_blast_strength"][rev_pos]
            == obs["bomb_blast_strength"][bomb_pos]
...
def _all_bomb_real_life(board, bomb_life, bomb_blast_st):
        ...
        for d in dirs:
            ...
            while True:
                ...
                if bomb_real_life[pos] > 0:
                    if (
                        bomb_real_life[pos] < min_life
                        and _manhattan_distance(pos, last_pos) <= bomb_blast_st[pos] - 1
                    ):
    ...
    while True:
        ...
        for i in range(sz):
            for j in range(sz):
                if (
                    utility.position_is_wall(board, (i, j))
                    or utility.position_is_powerup(board, (i, j))
...
def _position_covered_by_bomb(obs, pos, bomb_real_life_map):
    ...
    if obs["bomb_life"][pos] > 0:
        min_bomb_value, max_bomb_value = (
            bomb_real_life_map[pos],
            bomb_real_life_map[pos],
        )
    ...
    for d in dirs:
        ...
        while True:
            ...
            if _stop_condition(board, next_pos, exclude_agent=True):
                ...
                break
            if obs["bomb_life"][next_pos] > 0 and obs["bomb_blast_strength"][
                next_pos
            ] - 1 >= _manhattan_distance(pos, next_pos):
...
def _compute_min_evade_step(obs, parent_pos_list, pos, bomb_real_life):
    flag_cover, min_cover_value, max_cover_value = _position_covered_by_bomb(
        obs, pos, bomb_real_life
    )
    if not flag_cover:
        return 0
        ...
        for d in dirs:
            ...
            if not utility.position_on_board(board, next_pos):
                continue
            if not (
                utility.position_is_passage(board, next_pos)
                or utility.position_is_powerup(board, next_pos)
            ):
                continue
            if next_pos in parent_pos_list:
                continue
            x = _compute_min_evade_step(
                obs, parent_pos_list + [next_pos], next_pos, bomb_real_life
            )
...
def _check_if_flame_will_gone(obs, prev_two_obs, flame_pos):
    ...
    if not (
        utility.position_is_flames(prev_two_obs[0]["board"], flame_pos)
    ...
    while len(Q) > 0:
        pos = Q.popleft()
        if not (
            utility.position_is_flames(prev_two_obs[0]["board"], pos)
            and utility.position_is_flames(prev_two_obs[1]["board"], pos)
        ):
            return False
        for d in dirs:
            next_pos = utility.get_next_position(pos, d)
            if utility.position_on_board(board, next_pos) and utility.position_is_agent(
                board, next_pos
            ):
...
def _compute_safe_actions(obs, exclude_kicking=False, prev_two_obs=(None, None)):
    dirs = _all_directions(exclude_stop=True)
    ret = set()
    my_position, board, blast_st, bomb_life, can_kick = (
        obs["position"],
        obs["board"],
    ...
    for direction in dirs:
        ...
        if (
            (not exclude_kicking)
            and utility.position_in_items(board, position, [constants.Item.Bomb])
            and can_kick
        ):
        ...
        gone_flame_pos = None
        if (
            prev_two_obs is not None
            and prev_two_obs[0] is not None
            and prev_two_obs[1] is not None
            and _check_if_flame_will_gone(obs, prev_two_obs, position)
        ):
        ...
        if utility.position_is_passage(board, position) or utility.position_is_powerup(
            board, position
        ):
            my_id = obs["board"][my_position]
            obs["board"][my_position] = (
                constants.Item.Bomb.value
                if bomb_life[my_position] > 0
                else constants.Item.Passage.value
            )
            flag_cover, min_cover_value, max_cover_value = _position_covered_by_bomb(
                obs, position, bomb_real_life_map
            )
            flag_cover_passages.append(flag_cover)
            if not flag_cover:
                ret.add(direction.value)
            else:
                min_escape_step = _compute_min_evade_step(
                    obs, [position], position, bomb_real_life_map
                )
    ...
    flag_cover, min_cover_value, max_cover_value = _position_covered_by_bomb(
        obs, my_position, bomb_real_life_map
    )
    if flag_cover:
        min_escape_step = _compute_min_evade_step(
            obs, [None, my_position], my_position, bomb_real_life_map
        )
        if min_escape_step < min_cover_value:
    ...
    if not (obs["ammo"] <= 0 or obs["bomb_life"][obs["position"]] > 0):
        ...
        if BOMBING_TEST == "simple":
            ...
            if (
                (constants.Action.Stop.value in ret)
                and len(ret) > 1
                and (kick_dir is None)
            ):
                ...
                bomb_life2, bomb_blast_st2, board2 = (
                    obs2["bomb_life"],
                    obs2["bomb_blast_strength"],
                    obs2["board"],
                )
                bomb_real_life_map = _all_bomb_real_life(
                    board2, bomb_life2, bomb_blast_st2
                )
                min_evade_step = _compute_min_evade_step(
...
def get_filtered_actions(obs, prev_two_obs=None):
    ...
    ret = _compute_safe_actions(
        obs, exclude_kicking=NO_KICKING, prev_two_obs=prev_two_obs
    )
    obs = obs_cpy
    if len(ret) != 0:
        return list(ret)
...
def _kick_test(board, blast_st, bomb_life, my_position, direction):
    def moving_bomb_check(moving_bomb_pos, p_dir, time_elapsed):
        ...
        for i in range(10):
            ...
            if not utility.position_on_board(board, pos2):
                break
            if not (
                utility.position_is_powerup(board, pos2)
                or utility.position_is_passage(board, pos2)
            ):
                break
    ...
    for i in range(life_value):
        if utility.position_on_board(board, pos) and utility.position_is_passage(
            board, pos
        ):
            # do a check if this position happens to be in flame when the moving bomb arrives!
            if not (
                moving_bomb_check(pos, perpendicular_dirs[0], i)
                and moving_bomb_check(pos, perpendicular_dirs[1], i)
            ):
                break
            dist += 1
        else:
