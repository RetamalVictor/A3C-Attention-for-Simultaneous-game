...
class AgentLoss(nn.Module):
    """
    Loss module for agent training. More information about the
    loss mathematical background in:

    Guiding Simultaneous Move Monte Carlo Tree Search via Opponent Models
    Aljabasini, O. (Auteur). 16 aug. 2021
    """
    ...
    def agent_loss_func(
        self, agent_rewards, agent_log_probs, agent_values, agent_entropies
    ):
        policy_loss = 0
        value_loss = 0
        R = agent_values[-1]
        ...
        for i in reversed(range(len(agent_rewards))):
            # Calculation of Policy Loss function
            delta_t = (
                agent_rewards[i] + self.gamma * agent_values[i + 1] - agent_values[i]
            )
            gae = gae * self.gamma * self.gae_lambda + delta_t
            policy_loss = (
                policy_loss
                - agent_log_probs[i] * gae.detach()
                - self.entropy_coef * agent_entropies[i]
            )

            # Calculation of Value Loss function
    ...
    def opponent_loss_func(
        self, opponent_log_probs, opponent_actions_ground_truths, opponent_coefs
    ):
        """
        ...
        for i in range(nb_opponents):
            # policy loss
            policy_loss = policy_loss + opponent_coefs[i] * F.cross_entropy(
                opponent_log_probs[i], opponent_actions_ground_truths[i]
            )
        return policy_loss
    ...
    def forward(
        ...
        agent_policy_loss, agent_value_loss = self.agent_loss_func(
            agent_rewards, agent_log_probs, agent_values, agent_entropies
        )
        opponent_policy_loss = self.opponent_loss_func(
            opponent_log_probs, opponent_actions_ground_truths, opponent_coefs
        )
