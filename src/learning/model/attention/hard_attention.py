...
class HardAttention(nn.Module):
    def __init__(self, latent_dim, hard_attention_rnn_hidden_size, approximate=True):
        ...
        if self.hard_attention_rnn_hidden_size is not None:
            ...
            self.lstm = nn.LSTM(
                latent_dim * 2, hard_attention_rnn_hidden_size, bidirectional=True
            )
    ...
    def forward(self, agent_latent, opponent_latents):
        ...
        if self.hard_attention_rnn_hidden_size is not None:
            opponent_latents = torch.cat(
                (agent_latent.unsqueeze(0), opponent_latents), dim=0
            )
            # (nb_opponents + 1, batch_size, latent_dim)
            agent_latent_stacked_repeated = agent_latent.unsqueeze(0).repeat(
                opponent_latents.shape[0], 1, 1
            )
            # (nb_opponents + 1, batch_size, latent_dim * 2)
            agent_opponent_stacked = torch.cat(
                (agent_latent_stacked_repeated, opponent_latents), dim=-1
            )
            ...
            # (batch_size, nb_opponents + 1, 2)
            hard_attention = F.gumbel_softmax(
                hard_attention, tau=0.01, hard=self.hard, dim=-1
            )
            # (batch_size, nb_opponents + 1)
            hard_attention = hard_attention[..., 1]
            return hard_attention
        return torch.ones(
            agent_latent.shape[0],
            opponent_latents.shape[0] + 1,
