...
from learning.model.attention.multihead_soft_attention import MultiheadSoftAttention
...
class AttentionModel(nn.Module):
    def __init__(
        ...
        if self.nb_soft_attention_heads is not None:
            ...
            self.multihead_soft_attention = MultiheadSoftAttention(
                latent_dim=latent_dim,
    ...
    def forward(self, agent_latent, opponent_latents):
        if self.nb_soft_attention_heads is None:
            ...
            scores = torch.full((agent_latent.shape[0], ll), 1 / ll).to(
                agent_latent.device
            )
        ...
        attention_output, attention_scores = self.multihead_soft_attention(
